...
| `chunk_size` | int / object | Chunk size in bytes. May be a fixed integer (e.g. `2097152`) or an object with per-type rules (e.g. `{"default": 2097152, "max": 8388608}`). Extensions MAY define additional chunking strategies. |
...
| `push_peers` | bool / object | Whether REGISTRY proactively informs about new peers or waits for want. If object, MAY specify delivery details: `{"enabled": true, "want_ttl": 300, "max_peers_per_push": 10}` |
...
1. **`want` batch:** Core `want` takes a single resource_id. Batch want (multiple resource_ids, directory hashes, collection identifiers) is an extension concern — different networks will have different notions of what a "collection" is. A future extension MAY define batch want semantics, including wants for encrypted directory hashes or collection manifests.

2. **Push vs pull:** The manifest decides via `push_peers` field (section 4.2). Push requires REGISTRY to maintain a `wants` map — details of want expiration, un-want, and delivery semantics are part of the manifest's data delivery configuration. The manifest SHOULD be extended to specify delivery details when `push_peers: true` (e.g. TTL for want subscriptions, max peers per push).

3. **Resource limit:** The spec does not impose a limit. REGISTRY implementations MAY enforce limits as they see fit (rate limiting, max message size, connection throttling). The `announce_mode: per-server` option (section 4.2) inherently solves the large-library problem — peers announce as hosts, not file lists. Per-resource mode trackers handle limits at the implementation level.

4. **Discovery + signaling:** Defined together in this KIP as a single protocol with clearly separated interfaces (section 2 vs section 3). The spec allows implementations to split REGISTRY and SIGNALER into separate endpoints or services — they are separate roles that happen to share a connection in the reference implementation. An implementation MAY run SIGNALER on a different host, use a third-party STUN/TURN server, or omit signaling entirely if the transport doesn't need it.

5. **connection_hint format:** Free-form string. The format is a convention between REGISTRY and its PEERs, not a protocol-level constraint. Different discovery implementations will produce different hints (a KIP tracker says `webrtc:signal-required`, mDNS gives an IP+port, a BT bridge gives a TCP address). Implementations SHOULD document their hint format. A future extension MAY formalize a hint grammar if interoperability between discovery providers becomes necessary.
